...
func AddUser(c *gin.Context) {
	...
	_, _, err = client.Collection("users").Add(ctx, map[string]interface{}{
		...
		"school":   user.School,
		"is_admin": user.IsAdmin,   // Store is_admin flag
		"is_master": user.IsMaster, // Store is_master flag
	})
	if err != nil {
...
// TestFirestoreConnection tests Firestore connectivity by reading a sample document
func TestFirestoreConnection(c *gin.Context) {
	ctx := context.Background()

	// Attempt to read a single document from the "users" collection
	iter := client.Collection("users").Limit(1).Documents(ctx)
	doc, err := iter.Next()
	if err == iterator.Done {
		c.JSON(http.StatusOK, gin.H{"message": "Connected to Firestore, but no users found"})
		return
	}
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to connect to Firestore", "details": err.Error()})
		return
	}

	c.JSON(http.StatusOK, gin.H{"message": "Connected to Firestore", "sampleUser": doc.Data()})
}
